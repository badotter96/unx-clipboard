...
from PyQt5.QtCore import Qt, QUrl, pyqtSignal, QTimer, QRect, QPoint, QThread
...
class SettingsDialog(QDialog):
    def __init__(self, config, callbacks, parent=None):
        ...
        self.auto_sync.setChecked(self.config['sync'].get('auto_sync', False))
        self.sync_layout.addRow(self.auto_sync)
        ...
        self.sync_layout.addRow("Local Sync Folder Path:", self.local_sync_path_widget)
    ...
    def save_and_close(self):
        ...
        self.config['sync']['backend'] = 'LocalFolder' if self.local_sync_path_input.text() else 'None'
        
        self.app_callbacks['save_config'](self.config)
        self.accept()

    def clear_history(self):
        reply = QMessageBox.question(self, 'Confirm', "Are you sure you want to clear all non-essential items?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
...
class SnippingWidget(QWidget):
    screenshot_taken = pyqtSignal(str) # Emits the relative path of the screenshot
    ...
    def take_screenshot(self):
        ...
        # The path is relative to the 'images' folder in AppData
...
class DrawingCanvas(QWidget):
    """A custom QWidget that handles layered drawing with correct coordinate mapping and undo/redo."""
    ...
    def draw_line_to(self, end_pos):
        ...
        if self.tool == 'highlighter':
            color = QColor(self.pen_color); color.setAlpha(100); pen.setColor(color); pen.setWidth(15)
        painter.setRenderHint(QPainter.Antialiasing); painter.setPen(pen)
    ...
    def apply_crop(self):
        ...
        self.add_undo_state() # Save the state before cropping
        
        self.base_pixmap = self.base_pixmap.copy(self.crop_rect)
        
        cropped_drawing = self.drawing_pixmap.copy(self.crop_rect)
        self.drawing_pixmap = QPixmap(self.base_pixmap.size())
        self.drawing_pixmap.fill(Qt.transparent)
        painter = QPainter(self.drawing_pixmap)
        painter.drawPixmap(0,0, cropped_drawing)
        painter.end()
    ...
    def set_tool(self, tool_name):
        self.tool = tool_name; self.crop_rect = None; self.preview_pixmap.fill(Qt.transparent); self.update()
...
class ImageEditorDialog(QDialog):
    def __init__(self, image_path, parent=None):
        ...
        self.btn_draw = QPushButton("Draw"); self.btn_draw.setCheckable(True); self.btn_draw.setChecked(True)
        self.btn_draw.clicked.connect(lambda: self.set_tool('pen'))
        
        self.btn_highlight = QPushButton("Highlight"); self.btn_highlight.setCheckable(True)
        self.btn_highlight.clicked.connect(lambda: self.set_tool('highlighter'))
        
        self.btn_rect = QPushButton("Outline"); self.btn_rect.setCheckable(True)
        self.btn_rect.clicked.connect(lambda: self.set_tool('rect'))

        self.btn_crop = QPushButton("Crop"); self.btn_crop.setCheckable(True)
        self.btn_crop.clicked.connect(lambda: self.set_tool('crop'))
        
        self.btn_color = QPushButton("Color"); self.btn_color.clicked.connect(self.pick_color)
        
        self.btn_apply_crop = QPushButton("Apply Crop"); self.btn_apply_crop.hide()
        self.btn_apply_crop.clicked.connect(self.apply_crop_action)

        toolbar.addWidget(self.btn_draw); toolbar.addWidget(self.btn_highlight)
        toolbar.addWidget(self.btn_color); toolbar.addWidget(self.btn_rect); toolbar.addWidget(self.btn_crop)
        toolbar.addWidget(self.btn_apply_crop)
        toolbar.addStretch()

        self.btn_undo = QPushButton("Undo"); self.btn_undo.clicked.connect(self.undo_action)
        toolbar.addWidget(self.btn_undo)
        self.btn_redo = QPushButton("Redo"); self.btn_redo.clicked.connect(self.redo_action)
...
class AppGUI(QMainWindow):
    def __init__(self, db, icon_path):
        ...
        self.tab_widgets = {} # For lazy loading
        self.current_page = 1
        self.total_pages = 1
        self.items_per_page = 50 # A good default for performance
    ...
    def set_callbacks(self, callbacks):
        ...
        QTimer.singleShot(50, lambda: self.on_main_tab_changed(0))
    ...
    def create_widgets(self):
        ...
        self.main_tabs.addTab(QWidget(), "Clipboard")
        self.main_tabs.addTab(QWidget(), "Surfing")
        self.main_tabs.addTab(QWidget(), "Password Generator")
        self.main_tabs.addTab(QWidget(), "Screenshot")
    ...
    def on_main_tab_changed(self, index):
        if index not in self.tab_widgets:
            ...
            if tab_name == "Clipboard":
                self.create_clipboard_tab(index)
            elif tab_name == "Surfing":
                self.create_surfing_tab(index)
            elif tab_name == "Password Generator":
                self.create_password_gen_tab(index)
            elif tab_name == "Screenshot":
                self.create_screenshot_tab(index)
        
        if hasattr(self, 'surfing_widget'):
            surfing_tab_index = -1
            for i, widget in self.tab_widgets.items():
                if widget == self.surfing_widget:
                    surfing_tab_index = i
                    break
            
            if surfing_tab_index != -1:
                if index == surfing_tab_index:
                    self.surfing_widget.wake_browser()
                else:
                    self.surfing_widget.hibernation_timer.start(60000)

    def create_clipboard_tab(self, index):
        history_widget = QWidget()
        history_layout = QVBoxLayout(history_widget)
        self.tabs = QTabWidget(); self.list_widgets = {}
        ...
        for name in tab_names:
            ...
            self.tabs.addTab(list_widget, name); self.list_widgets[name] = list_widget
        ...
        pagination_layout.addStretch(); pagination_layout.addWidget(self.page_label); pagination_layout.addStretch()
        ...
        self.new_snippet_button = QPushButton("New Snippet"); self.new_snippet_button.clicked.connect(self.open_new_snippet_dialog)
        self.pin_button = QPushButton("Pin/Unpin"); self.pin_button.clicked.connect(lambda: self.perform_action_on_selected('pin'))
        self.delete_button = QPushButton("Delete"); self.delete_button.clicked.connect(lambda: self.perform_action_on_selected('delete'))
        button_layout.addWidget(self.new_snippet_button); button_layout.addWidget(self.pin_button); button_layout.addWidget(self.delete_button)
        history_layout.addWidget(self.action_buttons_widget)

        self.main_tabs.widget(index).setLayout(history_layout)
        self.tab_widgets[index] = history_widget
        self.populate_all_lists()

    def create_surfing_tab(self, index):
        self.surfing_widget = SurfingWidget()
        layout = QVBoxLayout(self.main_tabs.widget(index))
        layout.setContentsMargins(0,0,0,0)
        layout.addWidget(self.surfing_widget)
        self.tab_widgets[index] = self.surfing_widget

    def create_password_gen_tab(self, index):
        self.password_widget = PasswordGeneratorWidget()
        if self.app_callbacks: self.password_widget.copy_callback = self.app_callbacks['copy_and_log_text']
        layout = QVBoxLayout(self.main_tabs.widget(index))
        layout.setContentsMargins(0,0,0,0)
        layout.addWidget(self.password_widget)
        self.tab_widgets[index] = self.password_widget

    def create_screenshot_tab(self, index):
        ...
        snipping_info_label.setWordWrap(True); snipping_info_label.setAlignment(Qt.AlignCenter)
        self.snipping_widget_button = QPushButton("Launch Screenshot Tool")
        if self.app_callbacks: self.snipping_widget_button.clicked.connect(self.app_callbacks['start_snipping_tool'])
        snipping_layout.addWidget(snipping_info_label); snipping_layout.addWidget(self.snipping_widget_button, 0, Qt.AlignCenter)
        self.main_tabs.widget(index).setLayout(snipping_layout)
        self.tab_widgets[index] = snipping_tab_widget
    ...
    def populate_all_lists(self):
        ...
        self.list_widgets["All"].addItem("Loading history...")
        search_text = self.search_input.text()
        self.thread = QThread()
        self.worker = DataLoader(self.db, search_text, self.current_page, self.items_per_page)
        self.worker.moveToThread(self.thread)
        self.worker.results_ready.connect(self._on_data_loaded)
    ...
    def _on_data_loaded(self, entries, total_count):
        for lw in self.list_widgets.values(): lw.clear()
        if total_count == 0: self.total_pages = 1
        else: self.total_pages = math.ceil(total_count / self.items_per_page)
        for entry_id, content, content_type, timestamp, pinned, is_snippet in entries:
            pin_char = "📌" if pinned else " "; snippet_char = "🔖" if is_snippet else ""
            display_text = str(content or '').replace('\n', ' ').strip()
    ...
    def open_new_snippet_dialog(self):
        ...
        if ok and text: self.app_callbacks['add_new_snippet'](text)
    ...
    def create_full_backup(self):
        ...
        if filepath: self.app_callbacks['importer_exporter'].export_full_backup(filepath)
            
    def restore_from_backup(self):
        filepath, _ = QFileDialog.getOpenFileName(self, "Restore from Backup", "", "UNX Backup Files (*.unxbackup)")
        if filepath:
    ...
    def update_screenshot_preview(self, image_path):
        if hasattr(self, 'screenshot_preview_label'):
            ...
            if pixmap.isNull():
                ...
                scaled_pixmap = pixmap.scaled(self.screenshot_preview_label.size(), Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def show_item_context_menu(self, position):
        ...
        context_menu.addSeparator(); delete_action = context_menu.addAction("Delete Item")
    ...
    def on_item_select(self, current_item, previous_item=None):
        if not current_item:
            self.preview_text.hide(); self.preview_label.setText("Select a clipboard item to see a preview"); self.preview_label.show(); return
        ...
        if not entry: return
        content, content_type = entry; self.preview_text.hide(); self.preview_label.show()
        if content_type == 'text':
            self.preview_label.hide(); self.preview_text.setText(content); self.preview_text.show()
        elif content_type == 'image':
            full_path = os.path.join(USER_DATA_DIR, content)
            pixmap = QPixmap(full_path)
            if pixmap.isNull(): self.preview_label.setText(f"Image not found:\n{content}")
            else:
    ...
    def perform_action_on_selected(self, action_name):
        ...
        if not item: return
        entry_id = item.data(Qt.UserRole)
        if entry_id: self.app_callbacks[action_name](entry_id)
        
    def refresh_list(self):
        if hasattr(self, 'list_widgets'):
            self.populate_all_lists()
