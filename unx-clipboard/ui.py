...
class SettingsDialog(QDialog):
    def __init__(self, config, callbacks, parent=None):
        ...
        self.sync_backend.addItems(["None", "LocalFolder"]) # Only show relevant options
    ...
    def update_sync_fields_visibility(self, text):
        if 'stage_sync_provider' in self.app_callbacks:
            self.app_callbacks['stage_sync_provider'](text)
    ...
    def save_and_close(self):
        ...
        # The line for 'onedrive_client_id' is now correctly removed.
        self.app_callbacks['save_config'](self.config)
        self.accept()

    def clear_history(self):
        reply = QMessageBox.question(self, 'Confirm', "Are you sure you want to log out?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
...
class SurfingWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # --- NEW CACHE LOGIC ---
        # Define a path for the web cache inside our app's data folder
        web_cache_path = os.path.join(USER_DATA_DIR, "web_cache")
        os.makedirs(web_cache_path, exist_ok=True)
        
        # Get the default profile and set its cache path
