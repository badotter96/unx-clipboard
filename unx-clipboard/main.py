...
from config import (
    ...
    USER_DATA_DIR, USER_ICON_PATH, ICON_SOURCE_PATH, resource_path
)
...
class ClipboardApp:
    ...
    def copy_last_item(self):
        # This function was removed in a previous refactor but is needed by the tray icon
    ...
    def _on_new_entry(self, content, content_type):
        if not self.config['history'].get('log_images', True) and content_type == 'image':
            return
    ...
    def take_active_window_screenshot(self):
        """Captures the currently active window with enhanced stability."""
        try:
    ...
    def edit_image(self, entry_id):
        """Opens an existing image in the editor and saves the changes."""
